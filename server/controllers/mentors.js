...
const viewAssignedStudents = async (req, res) => {
  try {
    res
      .status(200)
      .json({ success: true, msg: "View Assigned Students Route" });
  } catch (error) {
    console.error(`Error: ${error.message}`);
    res
      .status(400)
  ...
  try {
    ...
    try {
      ...
      if (updatedStudent) {
        return res
          .status(200)
          .json({ success: true, msg: "Add Progress Route" });
      } else {
...
const studentEvaluation = async (req, res) => {
  try {
    ...
    form
      .getTextField("department_name")
      .setText(evaluateDetails.department_name);
    ...
    form
      .getTextField("internship_title")
      .setText(evaluateDetails.project_title);
    form.getTextField("work_done").setText(evaluateDetails.work_done);
    form
      .getTextField("report_quality_marks")
      .setText(evaluateDetails.report_quality_marks.scored);
    form
      .getTextField("oral_presentation_marks")
      .setText(evaluateDetails.oral_presentation_marks.scored);
    form
      .getTextField("work_quality_marks")
      .setText(evaluateDetails.work_quality_marks.scored);
    form
      .getTextField("work_understanding_marks")
      .setText(evaluateDetails.work_understanding_marks.scored);
    form
      .getTextField("periodic_interaction_marks")
      .setText(evaluateDetails.periodic_interaction_marks.scored);

    const reportQualityMarks = parseInt(
      evaluateDetails.report_quality_marks.scored
    );
    const oralPresentationMarks = parseInt(
      evaluateDetails.oral_presentation_marks.scored
    );
    const workQualityMarks = parseInt(
      evaluateDetails.work_quality_marks.scored
    );
    const workUnderstandingMarks = parseInt(
      evaluateDetails.work_understanding_marks.scored
    );
    const periodicInteractionMarks = parseInt(
      evaluateDetails.periodic_interaction_marks.scored
    );

    
    if (evaluateDetails.evaluation_name == 'ISE') {
      if (reportQualityMarks > 20) {
        return res.status(400).json({
          success: false,
          msg: 'Report Quality Marks cannot exceed 20',
        });
      }
      if (oralPresentationMarks > 30) {
        return res.status(400).json({
          success: false,
          msg: 'Oral Presentation Marks cannot exceed 30',
        });
      }
      if (workQualityMarks > 15) {
        return res.status(400).json({
          success: false,
          msg: 'Work Quality Marks cannot exceed 15',
        });
      }
      if (workUnderstandingMarks > 15) {
        return res.status(400).json({
          success: false,
          msg: 'Work Understanding Marks cannot exceed 15',
        });
      }
      if (periodicInteractionMarks > 20) {
        return res.status(400).json({
          success: false,
          msg: 'Periodic Interaction Marks cannot exceed 20',
        });
      }
    } else if (evaluateDetails.evaluation_name == 'ESE') {
      if (reportQualityMarks > 50) {
        return res.status(400).json({
          success: false,
          msg: 'Report Quality Marks cannot exceed 50',
        });
      }
      if (oralPresentationMarks > 40) {
        return res.status(400).json({
          success: false,
          msg: 'Oral Presentation Marks cannot exceed 40',
        });
      }
      if (workQualityMarks > 20) {
        return res.status(400).json({
          success: false,
          msg: 'Work Quality Marks cannot exceed 20',
        });
      }
      if (workUnderstandingMarks > 20) {
        return res.status(400).json({
          success: false,
          msg: 'Work Understanding Marks cannot exceed 20',
        });
      }
      if (periodicInteractionMarks > 20) {
        return res.status(400).json({
    ...
    const sumOfMarks =
      reportQualityMarks +
      oralPresentationMarks +
      workQualityMarks +
      workUnderstandingMarks +
      periodicInteractionMarks;
    ...
    form
      .getTextField("examiner_specific_remarks")
      .setText(evaluateDetails.examiner_specific_remarks);
    ...
    try {
      if (evaluateDetails.evaluation_name == "ISE") {
        ...
        const marksData = {
          evaluation: "ISE",
          report_quality_marks: evaluateDetails.report_quality_marks,
          oral_presentation_marks: evaluateDetails.oral_presentation_marks,
          work_quality_marks: evaluateDetails.work_quality_marks,
          work_understanding_marks: evaluateDetails.work_understanding_marks,
          periodic_interaction_marks:
            evaluateDetails.periodic_interaction_marks,
          total_marks: { outOf: 100, scored: sumOfMarks },
          pdf_buffer: finalPdfBuffer,
          examiner_specific_remarks: evaluateDetails.examiner_specific_remarks,
          exam_date: evaluateDetails.exam_date,
          exam_time: evaluateDetails.exam_time,
          exam_venue: evaluateDetails.exam_venue,
        };
        ...
        const updatedStudent = await Student.findOneAndUpdate(
          ...
          {
            $set: {
              "internships.0.evaluation.0": marksData,
            },
          }
        );
        if (updatedStudent) {
        ...
        const marksData = {
          ...
          periodic_interaction_marks:
            evaluateDetails.periodic_interaction_marks,
          total_marks: { outOf: 150, scored: sumOfMarks },
          pdf_buffer: finalPdfBuffer,
          examiner_specific_remarks: evaluateDetails.examiner_specific_remarks,
          exam_date: evaluateDetails.exam_date,
          exam_time: evaluateDetails.exam_time,
        ...
        const updatedStudent = await Student.findOneAndUpdate(
          ...
          {
            $set: {
              "internships.0.evaluation.1": marksData,
            },
          }
        );
        if (updatedStudent) {
  ...
  try {
    ...
    if (evaluation == "ISE") {
      ...
      if (updatedStudent) {
        return res
          .status(200)
          .json({ success: true, msg: "Uploaded Student Evaluation" });
      } else {
        return res.status(400).json({
          success: false,
          msg: `Something Went Wrong ${error.message}`,
...
const getAllMentors = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, count: total, pagination, data: mentor });
  } catch (error) {
  ...
  try {
    ...
    if (existing_mentor.students.length > 0) {
      return res.status(200).json({
        success: false,
        msg: `Cannot Delete Mentor. First Unassign All the Students`,
      });
    }
    ...
    return res
      .status(200)
      .json({ success: true, msg: "Deleted Mentor Successfully" });
  } catch (error) {
    console.error(`Error: ${error.message}`);
    return res
      .status(500)
...
const removeFaculty = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, msg: "Deleted Faculty Successfully" });
  } catch (error) {
    console.error(`Error: ${error.message}`);
    return res
      .status(500)
