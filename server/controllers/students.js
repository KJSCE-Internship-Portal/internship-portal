...
function calculateWeeks(startDate, endDate) {
  ...
  while (current.isBefore(end)) {
    ...
    let weekEnd = current.clone().endOf("isoWeek").isBefore(end)
      ? current.clone().endOf("isoWeek")
      : end.clone();
...
const loginStudent = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ success: false, msg: `Something Went Wrong ${error.message}` });
  }
};
...
const registerStudent = async (req, res) => {
  try {
    ...
    // Parse internships if it's a string (from FormData)
    if (typeof student.internships === "string") {
      student.internships = JSON.parse(student.internships);
    }
    ...
    // Handle offer letter file if present
    if (req.file) {
      console.log("Offer letter file received:", {
        originalname: req.file.originalname,
        size: req.file.size,
        bufferLength: req.file.buffer?.length,
      });
      student.internships[0].offer_letter = {
        filename: req.file.originalname,
        buffer: req.file.buffer, // Store buffer in database
        uploadDate: new Date(),
      };
    }
    ...
    const dates = calculateWeeks(
      student.internships[0].startDate,
      student.internships[0].endDate
    );
    ...
    // Verify offer letter was saved
    if (req.file) {
      const savedStudent = await Student.findOne({ email: student.email });
      console.log("Offer letter saved verification:", {
        filename: savedStudent.internships[0].offer_letter?.filename,
        hasBuffer: !!savedStudent.internships[0].offer_letter?.buffer,
        bufferLength: savedStudent.internships[0].offer_letter?.buffer?.length,
      });
    }
    ...
    return res
      .status(200)
      .json({ success: true, msg: "Student Registration Route" });
  } catch (error) {
    console.error(`Error: ${error.message}`);
    res
      .status(400)
...
const getAllStudents = async (req, res) => {
  try {
    ...
    if (!student) {
      return res
        .status(401)
        .json({ success: false, msg: "There are no Students" });
    }
    return res
      .status(200)
      .json({ success: true, count: total, pagination, data: student });
  } catch (error) {
...
//TESTING_________________________________#########_________#################_________________________###############################__________________________BELOW________________
const getOneStudent = async (req, res) => {
  try {
    ...
    if (person) {
      ...
      res.status(400).json({ success: false, msg: "Student not found" });
    }
  } catch (error) {
    console.error(`Error: ${error.message}`);
    res
...
const updateStudent = async (req, res) => {
  try {
    ...
    // Handle null stipend_amount when status is unpaid
    if (allowedUpdates.internships && Array.isArray(allowedUpdates.internships)) {
      allowedUpdates.internships = allowedUpdates.internships.map((internship) => {
        // If stipend_status is unpaid and stipend_amount is empty, set to null
        if (internship.stipend_status === "unpaid" && (!internship.stipend_amount || internship.stipend_amount === "")) {
          internship.stipend_amount = null;
        }
        // If stipend_status is paid and stipend_amount is provided, keep it
        if (internship.stipend_status === "paid" && internship.stipend_amount) {
          internship.stipend_amount = internship.stipend_amount.toString();
        }
        return internship;
      });
    }
    ...
    res.status(400).json({
      success: false,
      msg: `Error updating student: ${error.message}`,
    });
  }
};

//TESTING_________________________________#########_________#################_________________________###############################__________________________ABOVE________________
...
const uploadCertificate = async (req, res) => {
  try {
    ...
    if (updatedStudent) {
      return res
        .status(200)
        .json({ success: true, msg: "Uploaded Student Certificate" });
    } else {
      return res
        .status(400)
...
const uploadReport = async (req, res) => {
  try {
    ...
    if (updatedStudent) {
      return res
        .status(200)
        .json({ success: true, msg: "Uploaded Internship Report" });
    } else {
      return res
        .status(400)
        .json({ success: false, msg: `Something Went Wrong` });
...
const downloadOfferLetter = async (req, res) => {
  try {
    ...
    const student = await Student.findOne({ sub_id });
    if (!student) {
      return res
        .status(404)
    ...
    if (
      ...
      console.log("Offer letter not found in internships");
      return res
        .status(404)
        .json({ success: false, msg: "Offer letter not found" });
    }

    const offerLetter = student.internships[0].offer_letter;
    console.log("Offer letter data:", {
      filename: offerLetter.filename,
      hasBuffer: !!offerLetter.buffer,
      bufferType: typeof offerLetter.buffer,
      bufferLength: offerLetter.buffer?.length,
      bufferConstructor: offerLetter.buffer?.constructor?.name,
      isBuffer: Buffer.isBuffer(offerLetter.buffer),
    });

    if (!offerLetter.buffer) {
      console.log("Buffer is empty or undefined");
      console.log("Full offerLetter object:", JSON.stringify(offerLetter, null, 2));
      return res
        .status(404)
        .json({ success: false, msg: "Offer letter buffer not found" });
    }

    const filename = offerLetter.filename || "offer_letter.pdf";
    let buffer = offerLetter.buffer;

    console.log("Buffer before conversion:", {
      isBuffer: Buffer.isBuffer(buffer),
      hasBufferProp: !!buffer.buffer,
      bufferLength: buffer?.length,
      bufferType: typeof buffer,
    });

    // Handle MongoDB Binary type or other formats
    if (buffer.buffer && (buffer.buffer instanceof ArrayBuffer || buffer.buffer instanceof Buffer)) {
      console.log("Converting from BSON Binary format");
      buffer = Buffer.from(buffer.buffer);
    } else if (typeof buffer === "object" && buffer !== null && !(buffer instanceof Buffer)) {
      console.log("Converting from object to Buffer");
      // Try to get the underlying data
      if (buffer._bsontype === "Binary" || buffer.type === "Buffer") {
    ...
    console.log("Buffer after conversion:", {
      isBuffer: Buffer.isBuffer(buffer),
      bufferLength: buffer?.length,
    });
    ...
    console.error(`Error downloading offer letter: ${error.message}`);
    console.error(error);
    res
      .status(400)
      .json({ success: false, msg: `Something Went Wrong : ${error.message}` });
