...
const handleLoginRequest = async (req, res) => {
  try {
    ...
    const grefreshToken = req.cookies?.grefreshToken;

    if (grefreshToken) {
      const payload1 = await handleRefreshLogin(grefreshToken);
...
const callbackCheck = async (req, res) => {
  ...
  try {
    ...
    if (found) {
      ...
      const redirectURL =
        process.env.CLIENT_URL + `/student/details?userInfo=${encodedUserInfo}`;
