...
const unassignAllStudents = async (req, res) => {
  try {
    const mentor_email = req.body.mentor_email;
    const mentor = await Mentor.findOne({ email: mentor_email }).exec();
    if (!mentor) {
      return res.status(200).json({ success: true, msg: 'Mentor not found' });
    }
    for (const student of mentor.students) {
      const studentDetails = await Student.findOne({ email: student.email });
      studentDetails.hasMentor = false;
    ...
    return res
      .status(200)
      .json({ success: true, msg: 'All students unassigned' });
  } catch (error) {
    console.error(`Error: ${error.message}`);
    return res
      .status(400)
...
const assignStudent = async (req, res) => {
  try {
    ...
    if (!student) {
      console.log('Roll no does not exist');
      return res
        .status(200)
    ...
    if (student.hasMentor) {
      console.log('Student already has mentor');
      return res.status(200).json({
        success: false,
    ...
    if (student.department !== coordinator_department) {
      console.warn(`Warning: Roll no ${rollno} belongs to a different department (${student.department}).`);
      return res.status(200).json({
        success: false,
    ...
    const isStudentAssigned = mentor.students.some(
      (mentorStudent) =>
        student.email === mentorStudent.email ||
        student.rollno === mentorStudent.rollno
    );

    if (isStudentAssigned) {
      console.log('Student already assigned to mentor');
      return res.status(200).json({
        success: false,
        msg: `Student ${student.rollno} already assigned to the ${mentor_email}.`,
      });
    }
...
const removeAssignedStudent = async (req, res) => {
  try {
    ...
    const assignedStudentIndex = mentor.students.findIndex(
