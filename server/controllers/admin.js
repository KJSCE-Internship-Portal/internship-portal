...
const postAnnouncement = async (req, res) => {
  try {
    const { department, sender, received_by, content, recipientType } = req.body;
    ...
    if (!savedAnnouncement) {
      return res
        .status(500)
        .json({ success: false, msg: "Announcement Cannot Be Posted" });
    }
...
const addCoordinator = async (req, res) => {
  try {
    ...
    var existing_coordinator = await Coordinator.findOne({
      email: req.body.email,
    }).exec();
    var existing_mentor = await Mentor.findOneAndUpdate(
      { email: req.body.email },
    ...
    if (existing_coordinator) {
      return res
        .status(201)
        .json({ success: false, msg: `Coordinator Already Exists` });
    } else if (!existing_mentor && !existing_coordinator) {
      await coordinator.save();
      return res
        .status(200)
        .json({ success: true, msg: "Coordinator Registered Successfully !" });
    } else if (existing_mentor && !existing_coordinator) {
      return res
        .status(500)
        .json({
          success: false,
          msg: "A Mentor with the same E-mail ID is already Registered !",
        });
    }
  } catch (error) {
    console.error(`Error: ${error.message}`);
...
const deleteCoordinator = async (req, res) => {
  try {
    ...
    if (!coordinator) {
      return res
        .status(404)
        .json({ success: false, msg: "Coordinator not found" });
    }
    ...
    return res
      .status(200)
      .json({ success: true, msg: "Coordinator deleted successfully" });
  } catch (error) {
    console.error(`Error: ${error.message}`);
    res
      .status(400)
...
const getAllAnnouncements = async (req, res) => {
  try {
    ...
    // Filter announcements based on recipientType and user role
    const userRole = req.user?.role;
    const recipientTypeFilter = [];

    // All announcements with all_users should be visible to everyone
    recipientTypeFilter.push({ recipientType: "all_users" });
    recipientTypeFilter.push({ recipientType: { $exists: false } }); // For backward compatibility

    // Filter based on user role
    if (userRole === "ADMIN") {
      recipientTypeFilter.push({ recipientType: "all_coordinators" });
      recipientTypeFilter.push({ recipientType: "all_mentors" });
      recipientTypeFilter.push({ recipientType: "all_students" });
    ...
    // Combine the parsed query with recipient type filter
    if (Object.keys(parsedQuery).length > 0) {
      parsedQuery = {
        $and: [
          parsedQuery,
          { $or: recipientTypeFilter }
        ]
      };
    } else {
      parsedQuery = { $or: recipientTypeFilter };
    }

    query = Announcement.find(parsedQuery);
    ...
    const total = await Announcement.countDocuments(parsedQuery);
    ...
    if (!announcements) {
      return res
        .status(401)
        .json({ success: false, msg: "There are no Announcements" });
    }

    return res
      .status(200)
      .json({ success: true, count: total, pagination, data: announcements });
  } catch (error) {
...
const getStatisticsAdmin = async (req, res) => {
  try {
    const studentsInAllDepartments = await Student.countDocuments({
      isActive: true,
    });
            ...
            $cond: {
              ...
              then: 0,
                ...
                $multiply: [
                  {
                    $divide: [
                      "$submittedDepartmentProgress",
                      "$totalDepartmentProgress",
                    ],
                  },
                  100,
                ],
    ...
    return res
      .status(200)
      .json({ success: true, msg: "Statistics Route", data });
  } catch (error) {
    console.error(`Error: ${error.message}`);
    return res
      .status(400)
...
const assignStudent = async (req, res) => {
  try {
    ...
    if (!student) {
      return res
        .status(200)
        .json({ success: false, msg: `Roll no ${rollno} doesn't Exist !` });
    }
    if (student.email != student_email) {
      return res
        ...
        .json({
          success: false,
          msg: `${rollno} and ${student_email} does not belong to same student`,
        });
    }
    if (student.hasMentor) {
      return res
        ...
        .json({
          success: false,
          msg: `Roll no ${rollno} is already assigned to ${student.mentor.name}`,
        });
    }
    if (student.department != coordinator_department) {
      return res
        ...
        .json({
          success: false,
          msg: `Roll no ${rollno} does not belong to your department`,
        });
    }
    ...
    if (!mentor) {
      return res
        ...
        .json({
          success: false,
          msg: `Mentor ${mentor_email} doesn't Exist !`,
        });
    }

    // Check if the student is already assigned to the mentor
    const isStudentAssigned = mentor.Student.some(
      (mentor_student) => student.sub_id === mentor_student.sub_id
    );
    ...
    if (isStudentAssigned) {
      return res
        ...
        .json({
          success: false,
          msg: `Student ${rollno} already assigned to the ${mentor_email}.`,
        });
    }
    ...
    return res
      .status(200)
      .json({ success: true, msg: "Student assigned successfully" });
  } catch (error) {
    console.error(`Error: ${error.message}`);
    return res
      .status(400)
...
const removeAssignedStudent = async (req, res) => {
  try {
    ...
    const assignedStudentIndex = mentor.Student.findIndex(
      (mentor_student) => student.sub_id === mentor_student.sub_id
    );
    ...
    if (assignedStudentIndex === -1) {
      return res
        ...
        .json({
          success: false,
          msg: `Student ${rollno} is not assigned to ${mentor_email}.`,
        });
    }
...
const addMentor = async (req, res) => {
  try {
    ...
    var existing_mentor = await Mentor.findOne({
      email: req.body.email,
    }).exec();
    ...
    if (existing_mentor) {
      if (!existing_mentor.isActive) {
        ...
        return res
          .status(200)
          .json({ success: true, msg: "Mentor Registered Successfully !" });
      } else {
        return res
          .status(201)
          .json({ success: false, msg: `Mentor Already Exists` });
      }
      ...
      return res
        ...
        .json({
          success: false,
          msg: "A student with the same E-mail ID is already Registered !",
        });
    }
  } catch (error) {
    console.error(`Error: ${error.message}`);
...
const addMentors = async (req, res) => {
  try {
    ...
    for (const mentorData of mentors) {
      const existing_mentor = await Mentor.findOne({
...
const getStatisticsCoordinator = async (req, res) => {
  try {
    ...
    const studentsInDepartment = await Student.countDocuments({
      department,
      isActive: true,
    });
    const assignedStudents = await Student.countDocuments({
    ...
    const completedInternshipsPercentage = await Student.aggregate([
      ...
      },
      {
        $project: {
          completedPercentage: {
            $multiply: [
              { $divide: ["$completedInternships", "$totalInternships"] },
              100,
            ],
          },
        },
...
const getAllCoordinators = async (req, res) => {
  try {
    ...
    if (!coordinator) {
      return res
        .status(401)
        .json({ success: false, msg: "There are no Coordinators" });
    }
    return res
      .status(200)
      .json({ success: true, count: total, pagination, data: coordinator });
  } catch (error) {
    console.log(`${error.message} (error)`.red);
    return res.status(400).json({ success: false, msg: error.message });
  }
