...
export const ThemeProvider = ({ children }) => {
  // Initialize from sessionStorage instead of always using initialTheme
  const [currentTheme, setCurrentTheme] = useState(() => {
    const savedTheme = sessionStorage.getItem('appTheme');
    return savedTheme === 'dark' ? darkTheme : initialTheme;
  });
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem('appTheme', isDark ? 'dark' : 'light');
  }, [currentTheme]);
