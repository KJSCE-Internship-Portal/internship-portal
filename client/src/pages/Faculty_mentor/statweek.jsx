...
const StatWeek = () => {
  ...
  function generateWeekURL(week) {
    if (week.status == "Submitted") {
      ...
      showToast(toast, "Error", "error", "Update Nsot yet Submitted");
    }
  }
      ...
      <Card>
        ...
        >
          <div className="flex flex-col h-[164px] md:h-auto items-start justify-start w-full">
            <div
              className={`bg-black-900_0c flex flex-col gap-[51px] items-left justify-start pb-[73px] md:pr-10 sm:pr-5 pr-[73px] w-full relative`}
            >
              <text
  ...
  return (
    <>
      <div
        className={`bg-${colors.secondary2} flex flex-col font-roboto items-center justify-start mx-auto w-full max-h-full py-6 px-4`}
      >
        <BackButton />
        {/* Mentor */}
