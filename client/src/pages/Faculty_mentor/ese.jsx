...
const handleFileChange = (e) => {
  ...
  // helper: safe open PDF from buffer
  const openPdfFromBuffer = (buffer) => {
    try {
      ...
      // Optionally revokeObjectURL later if needed
    } catch (err) {
...
const getUser = async () => {
  ...
  // fetch student + evaluation data and populate form
  const fetchData = async () => {
    try {
      ...
      if (!student) {
        console.warn("fetchData: no student returned");
        return;
      }
      ...
      // evaluation[1] should be ESE
      const evalESE = student.internships?.[0]?.evaluation?.[1] || {};

      // pdf buffer if exists
      if (evalESE?.pdf_buffer) {
        setPdfBuffer(evalESE.pdf_buffer);
        if (evalESE.pdf_buffer?.data && evalESE.pdf_buffer.data.length > 0) {
          setToFill(false);
        }
      }

      // workdone and signature
      if (evalESE?.work_done) {
        setWorkdone(evalESE.work_done);
      }
      if (evalESE?.student_sign) {
        setStudentSignature(evalESE.student_sign);
      }

      // scheduled_date: normalize to yyyy-mm-dd or empty string
      if (evalESE?.scheduled_date) {
        const iso = new Date(evalESE.scheduled_date)
          .toISOString()
          .split("T")[0];
        // handle invalid epoch sentinel (1970-01-01) by treating as empty
        setScheduledDate(iso === "1970-01-01" ? "" : iso);
        // also set the date input with scheduled date if valid
      ...
      if (evalESE?.report_quality_marks?.scored !== undefined) {
        setQReport(String(evalESE.report_quality_marks.scored));
      }
      if (evalESE?.oral_presentation_marks?.scored !== undefined) {
        setOral(String(evalESE.oral_presentation_marks.scored));
      }
      if (evalESE?.work_quality_marks?.scored !== undefined) {
        setQWork(String(evalESE.work_quality_marks.scored));
      }
      if (evalESE?.work_understanding_marks?.scored !== undefined) {
        setUnderstanding(String(evalESE.work_understanding_marks.scored));
      }
      if (evalESE?.periodic_interaction_marks?.scored !== undefined) {
        setInteraction(String(evalESE.periodic_interaction_marks.scored));
      }
      if (evalESE?.examiner_specific_remarks) {
        setRemarks(evalESE.examiner_specific_remarks);
        setCharCount(evalESE.examiner_specific_remarks.length);
      }
    } catch (err) {
      console.error("fetchData error (ESE):", err);
    }
  };
  ...
  try {
    ...
    const evaluationData = student.internships?.[0]?.evaluation?.[1];
  ...
  // handle submit of marks (same validations you had, kept them)
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        // re-fetch and open PDF
        fetchData();
        window.open(pdfUrl, "_blank");
      }, 800);
    } catch (err) {
      console.error("submit error (ESE):", err);
      showToast(toast, "Error", "error", "Failed to submit data");
    }
  };

  // Listen for evaluationDateChanged custom event (app-wide)
  useEffect(() => {
    const handler = (e) => {
      try {
        ...
        console.log("ESE received evaluationDateChanged:", detail);
        ...
        if (
          ...
          showToast(
            ...
            "Success",
            "success",
            `ESE Date updated: ${newDate}`
          );
          // optionally refetch entire data
          // fetchData();
        }
      } catch (err) {
        console.error("error handling evaluationDateChanged (ESE)", err);
      }
    };

    window.addEventListener("evaluationDateChanged", handler);
    return () => {
      window.removeEventListener("evaluationDateChanged", handler);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  useEffect(() => {
    const storageHandler = (e) => {
      try {
        ...
        const currentStudentId = localStorage.getItem("student");
        console.log("ESE received storage event:", parsed);
        if (
          evaluation === "ESE" &&
          String(student_id) === String(currentStudentId)
        ) {
          ...
          showToast(
            ...
            "Success",
            "success",
            `ESE Date updated (storage): ${newDate}`
          );
        }
      } catch (err) {
        console.error("storageHandler error (ESE):", err);
      }
    };
      ...
      {toFill || isEditingEvaluation ? (
        <div className="max-w-2xl mx-auto px-4">
          ...
          >
            ...
            {/* Date (this will show immediately when updated elsewhere) */}
            <div className="flex justify-between items-center mb-6">
              <h2
                className={`text-lg lg:text-2xl font-bold text-${colors.font}`}
              >
            ...
            <div className="py-2 px-4 mb-4 text-white rounded-lg rounded-t-lg border border-gray-200 dark:bg-gray-500 dark:border-gray-700">
              <input
                ...
                value={date}
                onChange={(e) => {
                  // keep both date and scheduledDate in sync if user edits manually
        ...
        /* View mode: upload file / view eval sheet */
        <div className="max-w-2xl mx-auto px-4">
          <BackButton to="/mentor/studentprogress" label="Back to student profile" />
