...
const InternshipPlatform = () => {
    ...
    try {
      ...
      if (userInfo) {
        ...
        const response = await axios.get(
          url + `/students/all?mentor.email=${userInfo.email}`
        );
            ...
            if (student) {
              if (student.isApproved) {
                ...
                student.isApproved = student.isApproved
                  ? "Approved"
                  : "Not Approved";
                setStudentsForApproval((prevStudent) => [
                  ...prevStudent,
                  student,
                ]);
              }
  ...
  const InternshipItem = ({
    ...
    return (
      <div>
        <Card
          className={`bg-white rounded-lg shadow-md p-4 min-w-full mb-4 mt-4 transform transition-transform hover:translate-y-[-2px] hover:shadow-md text-${colors.font}`}
        >
            ...
            <div className="flex space-x-2">
              {withButton && (
                <div className="flex space-x-2 items-end ml-full">
                  <button
                    className={`w-8 h-8 flex items-center justify-center bg-green-500 rounded-full cursor-pointer text-${colors.font}`}
                    onClick={onApprove}
                  >
                    <svg
                  ...
                  <button
                    className={`w-8 h-8 flex items-center justify-center bg-red-500 rounded-full cursor-pointer text-${colors.font}`}
          ...
          <div
            ...
            onClick={() =>
              status === true ? viewUser(student) : trackUser(student.sub_id)
            }
          >
            <Avatar
              size="md"
            ...
            <div>
              ...
              <div className="text-xs text-gray-500">
                Company Name {student.internships[0].company}
              </div>
            </div>
          </div>
        </Card>
  ...
  return (
    <div
      className={`bg-${colors.secondary2} flex flex-col font-roboto items-center justify-start mx-auto w-full max-h-full py-6 px-4 h-screen text-${colors.font}`}
    >
      <div className="flex md:flex-col flex-row gap-3 h-[70px] md:h-auto items-center justify-start max-w-[1262px] mx-auto pt-4 md:px-5 w-full mb-3.5">
        <div className="flex flex-row justify-start w-full">
          ...
          <div className="flex flex-1 flex-col items-start justify-start w-full">
            ...
            <p className={`text-${colors.font} text-xs w-full`}>
              {mentorEmail}
            </p>
          </div>
          <button
      ...
      />
      <h1
        className={`text-base text-${colors.font} w-full text-center font-bold`}
      >
        {mentorDepartment}
      </h1>
      ...
      <div className="flex-grow p-4 min-w-full">
        {currentStudents && currentStudents.length > 0 ? (
          <div className="mb-8">
            <h2 className={`text-xl font-bold text-${colors.font}`}>
              Assigned Students
            </h2>
          ...
          <div className={`text-xl font-bold text-${colors.font}`}>
          ...
          <div className="mb-8">
            ...
            {studentsForApproval.map((student) => (
              <InternshipItem
                ...
                status={true}
                onApprove={() =>
                  approveStudent(student.sub_id, true, student.mentor.email)
                }
                onDisapprove={() =>
                  approveStudent(student.sub_id, false, student.mentor.email)
                }
              />
            ))}
          </div>
