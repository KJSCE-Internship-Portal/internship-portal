...
const Comment = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (userInfo) {
          ...
          setWeekNo(weekNo);
          setStartDate(
            new Date(student.internships[0].progress[weekNo - 1].startDate)
              .toISOString()
              .substring(0, 10)
          );
          setEndDate(
            new Date(student.internships[0].progress[weekNo - 1].endDate)
              .toISOString()
              .substring(0, 10)
          );
          ...
          setWeekDescription(
            student.internships[0].progress[weekNo - 1].description
          );
          const privComment =
            student.internships[0].progress[weekNo - 1].mentor_comment;
  ...
  return (
    <section
      class={`bg-${colors.secondary} py-8 lg:py-16 antialiased h-screen`}
    >
      <div class="max-w-2xl mx-auto px-4">
        <BackButton to="/mentor/studentprogress" label="Back to student progress" />
        <div class="flex justify-between items-center mb-6">
          <h2 class={`text-lg lg:text-3xl font-bold text-${colors.font}`}>
            Weekly Report:
          </h2>
        </div>
        <div
          class={`flex justify-between items-center mb-6 text-${colors.font}`}
        >
        ...
        <article class={`p-6 text-base bg-${colors.secondary} rounded-lg`}>
          <footer class="flex justify-between items-center mb-2">
            <div class="flex items-center">
              <p
                class={`inline-flex items-center mr-3 text-sm text-${colors.font} font-semibold`}
              >
                <Avatar
                  size="xs"
                  bg="red.700"
          ...
          {!mentorProfilePicture && (
            <div class="flex items-center mt-4 space-x-4 mb-3">
              <button
                type="button"
                class={`flex items-center text-sm text-${colors.font} hover:underline font-medium`}
          ...
          {wantToReply && wantToReply === true ? (
            <form className="mb-6" onSubmit={handleSubmit}>
              <div
                className={`py-2 px-4 mb-4 bg-${colors.secondary} rounded-lg rounded-t-lg border border-${colors.accent}`}
              >
        ...
        {mentorProfilePicture && (
          <article
            class={`p-2 mb-3 ml-6 lg:ml-12 text-base bg-${colors.secondary} rounded-lg`}
          >
            <footer class="flex justify-between items-center mb-2">
              <div class="flex items-center">
                <p
