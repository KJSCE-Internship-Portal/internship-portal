...
const Dashboard = () => {
  ...
  const uploadExcel = async () => {
    try {
      const response = await axios.post(url + "/coordinator/add/mentors", {
        csvData,
  ...
  const filterColumns = (data) => {
    ...
    const filteredData = data.map((row, index) => {
      ...
      if (!row.name || row.name.trim() === "") {
        errors.push({
          field: "name",
          message: `Name is required for row ${index + 1}`,
        });
      }
      ...
      if (!row.email || !emailRegex.test(row.email)) {
        errors.push({
          field: "email",
          message: `Invalid or missing email for row ${
            index + 1
      ...
      if (!row.contact_no || !contactNoRegex.test(row.contact_no)) {
        errors.push({
          field: "contact_no",
          message: `Invalid or missing contact number for row ${
            index + 1
  ...
  const { isError, isLoading, data } = useQuery({
    ...
    queryFn: async () => {
      ...
      setPieData((x) => [
        ...x,
        ...temp.topCompanies.map((company) => [company._id, company.count]),
      ]);
  ...
  if (isError) {
    return (
      <div>Something Went Wrong, Try Reloading if the problem persists</div>
    );
  }
    ...
    <ChakraProvider theme={theme}>
      <Box maxW="1200px" mx="auto" py={5} px={2}>
        ...
        <SimpleGrid columns={{ base: 1, md: 3 }} spacing={5} mt={5} mb={5}>
          <Button colorScheme="red" onClick={viewstudent}>
            View Students
          </Button>
          <Button colorScheme="red" onClick={viewmentor}>
            View Mentors
          </Button>
          <Button colorScheme="red" onClick={viewcoord}>
            View Coordinators
          </Button>
        </SimpleGrid>
        ...
        <SimpleGrid columns={{ base: 1, md: 2, lg: 4 }} spacing={5}>
          ...
          <Stat bg="green.100" p={4} borderRadius="md">
            ...
            <StatNumber>
              {Number(data.avgInternshipDuration[0]?.avgDuration).toFixed(2)}
            </StatNumber>
          ...
          <Stat bg="orange.100" p={4} borderRadius="md">
            <StatLabel>Total students without Mentors</StatLabel>
            <StatNumber>{data.unassignedStudents}</StatNumber>
            <StatHelpText>Students not yet assigned to any mentor</StatHelpText>
            </Stat>
