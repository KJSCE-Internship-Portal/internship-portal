...
const Comment = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (userInfo) {
          ...
          setStartDate(
            new Date(userInfo.internships[0].progress[week - 1].startDate)
              .toISOString()
              .substring(0, 10)
          );
          setEndDate(
            new Date(userInfo.internships[0].progress[week - 1].endDate)
              .toISOString()
              .substring(0, 10)
          );
          ...
          setWeekDescription(
            userInfo.internships[0].progress[week - 1].description
          );
          const privComment =
            userInfo.internships[0].progress[week - 1].mentor_comment;
  ...
  return (
    ...
    >
      <div
        className={`max-w-2xl mx-auto px-4 text-${colors.font} dark:text-black`}
      >
        {" "}
        ...
        <div className="flex justify-between items-center mb-6">
          <h2
            className={`text-lg lg:text-3xl font-bold text-${colors.font} w-100`}
          >
            Weekly Report
          </h2>
        </div>
        <div
          className={`flex justify-between items-center mb-6 text-${colors.font}`}
        >
          <h2>
            From{" "}
            <span
        ...
        <article
          className={`p-6 text-base bg-${colors.secondary} rounded-lg mb-3`}
          style={{
            borderColor: `#${colors.font}`,
            boxShadow: `0 0 10px rgba(${254 / 2}, ${254 / 2}, ${254 / 2}, 0.2)`,
          }}
        >
        ...
        <article
          className={`p-2 mb-3 ml-6 lg:ml-12 text-base bg-${colors.secondary} rounded-lg`}
        >
          <footer className="flex justify-between items-center mb-2">
            <div className="flex items-center">
              <p
                className={`inline-flex items-center mr-3 text-sm text-${colors.font} font-semibold`}
              >
