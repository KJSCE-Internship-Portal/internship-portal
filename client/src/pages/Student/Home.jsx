...
const FramePage = () => {
  ...
  const [isCertificateNotSubmitted, setIsCertificateNotSubmitted] =
    useState(true);
    ...
    if (sign1 != "") {
      ...
      if (new Date(iseDate).toISOString().split("T")[0] == currentDate) {
        ...
        showToast(
          ...
          "error",
          `${
            new Date(iseDate).toISOString().split("T")[0] === "1970-01-01"
              ? `ISE Date not Set`
              : `Can submit ISE Work Done on ${new Date(
                  iseDate
    ...
    if (sign2 != "") {
      ...
      if (new Date(eseDate).toISOString().split("T")[0] == currentDate) {
        ...
        showToast(
          ...
          "error",
          `${
            new Date(eseDate).toISOString().split("T")[0] === "1970-01-01"
              ? `ESE Date not Set`
              : `Can submit ESE Work Done on ${new Date(
                  eseDate
                ).toLocaleDateString("en-US", {
                  month: "short",
                  day: "numeric",
  ...
  const fetchData = async () => {
    try {
      ...
      if (userInfo) {
        ...
        if (
          userInfo.internships[0].progress &&
          userInfo.internships[0].progress.length > 0
        ) {
          ...
          const updatedProgressData = userInfo.internships[0].progress
            ...
            .map((weekInfo, index) => {
              ...
              if (isSubmitted && !isLateSubmission) {
                setOnTimeSubmission((prevValue) =>
                  prevValue === null ? 1 : prevValue + 1
                );
              } else if (!isSubmitted) {
                setNoSubmission((prevValue) =>
                  prevValue === null ? 1 : prevValue + 1
                );
              }
              if (isSubmitted && isLateSubmission) {
                setLateSubmission((prevValue) =>
                  prevValue === null ? 1 : prevValue + 1
                );
              }
              return {
          ...
          setProgressValue(
            (
              (updatedProgressData.length /
                parseInt(userInfo.internships[0].duration_in_weeks)) *
              100
            ).toFixed(2)
          );
          ...
          console.log(
            (
              ((onTimeSubmission + lateSubmission) /
                parseInt(userInfo.internships[0].duration_in_weeks)) *
              100
            ).toFixed(2)
          );
          ...
          if (
            userInfo.internships[0].isCompleted &&
            userInfo.internships[0].completion[0]?.pdf_buffer
          ) {
            setIsCertificateNotSubmitted(false);
            setCertificatePdfBuffer(
              userInfo.internships[0].completion[0].pdf_buffer
            );
          }
          if (
            userInfo.internships[0].isSubmitted &&
            userInfo.internships[0].report[0]?.pdf_buffer
          ) {
            setIsReportNotSubmitted(false);
            setReportPdfBuffer(userInfo.internships[0].report[0].pdf_buffer);
          }
          if (
            userInfo.internships[0].isSubmittedOther &&
            userInfo.internships[0].othersubmissions[0]?.pdf_buffer
          ) {
  ...
  const WeekComponent = ({ week, generateWeekURL }) => {
    return (
      <Card>
        ...
        >
          <div
            className={`bg-${colors.secondary} flex flex-col h-[164px] md:h-auto items-start justify-start w-full`}
          >
            <div
              className={`bg-${colors.secondary} flex flex-col gap-[51px] items-left justify-start pb-[73px] md:pr-10 sm:pr-5 pr-[73px] w-full relative`}
            >
              <text
                className={`justify-center p-1 rounded-br-md rounded-tl-md text-xs font-semibold w-auto ${
                  week.description
                    ? week.late
                      ? "text-orange-600"
  ...
  return (
    <>
      <div
        className={`bg-${colors.secondary} flex flex-col font-roboto items-center justify-start mx-auto w-full max-h-full py-6 px-4`}
      >
        {/* User */}
        <div
          className={`flex md:flex-col flex-row gap-3 h-[100px] md:h-auto items-center justify-start max-w-[1262px] mx-auto pt-4 md:px-5 w-full mb-3.5`}
        >
          <div className="flex flex-row justify-start w-full">
            ...
            <button
              className="bg-red-500 hover hover:bg-red-800 text-white px-4 py-2 rounded-md"
              onClick={openDrawer}
            >
              View Profile
            </button>
        ...
        <div className="md:pl-6 mx-10 md:mt-3 mb:5 md:pr-6 min-w-full">
          <div className="flex flex-col gap-3">
            <Tooltip
              hasArrow
              label={`${weeksDone} out of ${totalWeeks} weeks done : ${progressValue}% Progress`}
            ...
            <Tooltip
              ...
              label={`${
                onTimeSubmission + lateSubmission
              } out of ${totalWeeks} weeks submitted : ${(
                ((onTimeSubmission + lateSubmission) / totalWeeks) *
                100
              ).toFixed(2)}% Progress`}
              placement="top-end"
            >
          ...
          <SimpleGrid
            ...
            className="mb-5 mt-2"
          >
            <Stat bg="green.500" p={4} borderRadius="md">
            ...
            <Stat bg="orange.100" p={4} borderRadius="md">
              ...
              <StatHelpText>
                Count of weekly updates submitted after week deadline{" "}
              </StatHelpText>
            </Stat>
          </SimpleGrid>
        </div>
              ...
              <div className="sm:flex-col flex-row gap-5 grid sm:grid-cols-2 md:grid-cols-2 grid-cols-1 justify-start w-full">
                {progressData.map((week) => (
                  <WeekComponent
                    key={week.week}
                    week={week}
                    generateWeekURL={generateWeekURL}
                  />
                ))}
                {/* {progressData.map((week) => (
              ...
              <div className="sm:flex-col flex-row gap-5 grid sm:grid-cols-2 md:grid-cols-2 grid-cols-1 justify-start w-full">
                <Card bg={colors.hover}>
                  ...
                  >
                    <div className="flex flex-col gap-1 items-start justify-start p-2 w-full">
                      <text
                        className={`text-${colors.font} font-semibold text-md w-full`}
                        size="txtRobotoRegular12Black900"
                      >
                        ISE Work Done
                      </text>
