...
const AssignStudent = ({ mentor_sub_id, mentor_email, onAssigned }) => {
    ...
    try {
      ...
      if (!listOfStudents) {
        stu = (
          await axios.get(
            url +
              "/students/all?department=" +
              slugify(current_user.department) +
              "&select=rollno,email,name,profile_picture_url&hasMentor=false"
          )
        ).data.data;
      ...
      >
        ...
        <DrawerContent color={colors.font} bg={colors.secondary}>
          ...
          <DrawerBody>
            ...
            <Stack
              maxH={"350px"}
              style={{ padding: "3px 5px 10px 7px" }}
              overflowY={"auto"}
            >
              {listOfStudents &&
                  ...
                  <Tooltip key={index} label={stu.name + ", " + stu.email}>
                    <div
                      style={{
                        ...
                        flexDirection: "row",
                        backgroundColor:
                          rollNo === stu.rollno ? colors.hover : null, // Check if the student is selected
                        cursor: "pointer",
                      }}
                      onClick={() => {
                        setEmail(stu.email);
