    ...
    try {
      const updatedData = await Promise.all(
        csvData.map(async (row) => {
          ...
          const studentCheck = await axios.post(
            url + "/coordinator/check-student",
            { email: Student_email }
          );
          console.log(studentCheck);
          const isStudentRegistered = studentCheck.data.isRegistered;

          const mentorCheck = await axios.post(
            url + "/coordinator/check-mentor",
            { email: Mentor_email }
          );
          ...
          return {
            ...row,
            assignStatus:
              isStudentRegistered && isMentorRegistered ? "assign" : "skip",
          };
        })
      );

      const dataToUpload = updatedData.filter(
        (row) => row.assignStatus === "assign"
      );

      if (dataToUpload.length > 0) {
        const response = await axios.post(
          url + "/coordinator/add-students-excel",
          {
            csvData: csvData,
  ...
  const handleAddStudent = async ({ student_email, mentor_email, roll_no }) => {
    ...
    if (studentEmailError || mentorEmailError || roll_no.length === 0) {
      ...
      showToast(
        ...
        "Error",
        "error",
        "Provide valid email(s) and roll number"
      );
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(
        url + "/coordinator/mentor/assign-student",
        {
          mentor_email,
    ...
    for (let i = 0; i < filteredData.length; i++) {
      ...
      if (processedEmails.has(Student_email) || processedRollNo.has(roll_no)) {
        ...
        showToast(
          ...
          "Error",
          "error",
          `Duplicate Email found for ${Student_email}`
        );
        continue;
      }
  ...
  const filterColumns = (data) => {
    ...
    const filteredData = data.map((row, index) => {
      ...
      if (!row.Student_name || row.Student_name.trim() === "") {
        errors.push({
          field: "Student_name",
          message: `Student Name is required for row ${index + 1}`,
        });
      }
      ...
      if (!row.Student_email || !emailRegex.test(row.Student_email)) {
        errors.push({
          field: "Student_email",
          message: `Invalid or missing Student Email for row ${
            index + 1
          }. It should be of @somaiya.edu domain.`,
        });
      }
      ...
      if (row.roll_no && !/^\d+$/.test(row.roll_no)) {
        errors.push({
          field: "roll_no",
          message: `Invalid Roll No for row ${
            index + 1
      ...
      if (row.Mentor_name && row.Mentor_name.trim() === "") {
        errors.push({
          field: "Mentor_name",
          message: `Mentor Name is required for row ${index + 1}`,
        });
      }
      ...
      if (row.Mentor_email && !emailRegex.test(row.Mentor_email)) {
        errors.push({
          field: "Mentor_email",
          message: `Invalid or missing Mentor Email for row ${
            index + 1
          }. It should be of @somaiya.edu domain.`,
        });
      }
    ...
    const validationErrors = [...uniqueErrors].map((errorString) =>
    ...
    <div>
      ...
      >
        ...
        {selectedFile ? (
          ...
          <p style={{ margin: "0" }}>
            Drag & drop your Excel (.xlsx) file here, or click to select one
          </p>
        )}
